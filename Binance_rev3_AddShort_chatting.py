...
"""
    ...
    # 종목을 새로 선택했을 때는 별도 입력 없이도 최초 1회 자동으로 전략 브리핑을 실행
    auto_prompt = None
    if st.session_state.auto_strategy_trigger and not st.session_state.chat_history:
    ...
    prompt_to_run = user_prompt or auto_prompt
    ...
    if prompt_to_run:
        ...
        with st.chat_message("assistant", avatar="🤖"):
            try:
                full_reply = st.write_stream(_stream_gemini_reply(prompt_to_run))
            except Exception as e:
                full_reply = f"⚠️ Gemini API 호출 오류: {e}"
                st.error(full_reply)
